...
const Us1BoardPage = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const res = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/major/Urban/posts/us1?page=${currentPage}&limit=10`);
  ...
  const handleWrite = () => {
    navigate('/board/major/us1/write');
  };

  const goToPage = (page) => {
    axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/major/Urban/posts/us1?page=${page}&limit=10`)
      .then(res => {
        setPosts(res.data.posts || []);
        ...
        <tbody>
          {posts.length > 0 ? (
            posts.map(post => (
              <tr key={post._id}>
                <td style={styles.td}>
                  <Link to={`/board/major/us1/${post._id}`} style={styles.link}>
                    {post.title}
                  </Link>
                </td>
