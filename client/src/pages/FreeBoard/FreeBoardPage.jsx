...
const FreeBoardPage = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const res = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/posts/free?page=${currentPage}&limit=10`);
  
      setPosts(res.data.posts || []); // 방어
  ...
  const goToPage = (page) => {
    axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/posts/free?page=${page}&limit=10`) // ✅ 이 URL로
      .then(res => {
        ...
        <tbody>
          {posts.length > 0 ? (
            posts.map(post => (
              <tr key={post._id}>
                <td style={styles.td}>
                  <Link to={`/board/free/${post._id}`} style={styles.link}>
                    {post.title}
                  </Link>
                </td>
